...
- Discover the current per-user OpenAI Codex executable when the app is
  ...
  performed in WSL.
- Pin Browserslist to a patched release for clean production dependency audits.
...
### Added
- Same product surface as `1.0.0-rc.1`; final version bump and release
  notes only.

### Known limitations
- Codex-only; no other provider, browser-cookie import, or notification
  surface.
- App Server protocol is experimental; tested with Codex CLI 0.146.0.
...
Release candidate build of the V1 Windows tray app. Same product surface as
`0.1.0-alpha.1`; version bump only for the RC artifact set.

---

## 0.1.0-alpha.1 - 2026-08-07

First V1 Windows release candidate surface for codex-barbar: a tray app that
shows Codex usage and limits through the official Codex App Server protocol.

### Added
- Tray panel and settings surfaces (React 18 + Tauri 2) with the V1 identity
  `codex-barbar` / `com.naipi11.codexbarbar`.
- Account layer: SQLite store, DPAPI Current User vault, isolated managed
  `CODEX_HOME` runtimes, recovery, and a read-only current CLI profile.
- Redacted rotating logs (5 MiB / 14 days) and fixed-location redacted
  diagnostic export.
- Per-user NSIS installer with default data-preserving uninstall and an
  explicit `--purge-user-data` confirmation; portable ZIP that stores data
  under `%LOCALAPPDATA%\codex-barbar`.
- Official GitHub Windows CI (PR gate + release workflow), deterministic
  SPDX 2.3 SBOM generation, fail-closed license audit, and artifact
  verification scripts.

### Changed
- Release graph reduced to the Codex provider and V1 surfaces only; legacy
  providers, browser-cookie import, FloatBar/PopOut, telemetry, and
  auto-update are removed.
- Docs rewritten for the V1 product, build, release, privacy, and
  troubleshooting surfaces.

### Removed
- Inno Setup / WiX packaging, the standalone CLI release surface, macOS
  cross-build wrapper, and non-English legacy READMEs.
